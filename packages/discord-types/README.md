...
While it was primarily created for Nexcord, other client mods could also benefit from this, so it is published as a standalone package!

## Installation

```bash
npm install -D @nexcord/discord-types
yarn add -D @nexcord/discord-types
pnpm add -D @nexcord/discord-types
```

## Example Usage

```ts
import type { UserStore } from "@nexcord/discord-types";
...
```ts
import { ApplicationCommandType } from "@nexcord/discord-types/enums";

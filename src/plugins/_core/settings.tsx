...
const settings = definePluginSettings({
    settingsLocation: {
        ...
        description: "Where to put the Nexcord settings section",
        options: [
    ...
    includeNexcordInfoWhenCopying: {
        type: OptionType.BOOLEAN,
        description: "Also copy Nexcord info (Nexcord, Electron, Chromium) when clicking the version info in the bottom left area of the Settings page",
        default: true
    }
});
...
export default definePlugin({
    ...
    buildLayout(originalLayoutBuilder: SettingsLayoutBuilder) {
        ...
        if (layout.some(s => s?.key === "nexcord_section")) return layout;

        const { buildEntry } = this;

        const nexcordEntries: SettingsLayoutNode[] = [
            buildEntry({
                key: "nexcord_main",
                title: "Nexcord",
                panelTitle: "Nexcord Settings",
                Component: NexcordTab,
            ...
            buildEntry({
                key: "nexcord_cloud",
                title: "Cloud",
                panelTitle: "Nexcord Cloud",
                Component: CloudTab,
                Icon: CloudIcon
            }),
            buildEntry({
                key: "nexcord_backup_restore",
            ...
            ...this.customSections.map((func, i) => {
                ...
                return buildEntry({
                    key: `nexcord_deprecated_custom_${section}`,
                    title: label,
                    Component: element,
        ...
        const nexcordSection: SettingsLayoutNode = {
            key: "nexcord_section",
            type: LayoutTypes.SECTION,
            useTitle: () => "Nexcord Settings",
            buildLayout: () => nexcordEntries
        };
        ...
        layout.splice(idx, 0, nexcordSection);
    ...
    get electronVersion() {
        return NexcordNative.native.getVersions().electron || window.legcord?.electron || null;
    },

    get chromiumVersion() {
        try {
            return NexcordNative.native.getVersions().chrome
    ...
    getInfoRows() {
        ...
        const rows = [`Nexcord ${gitHash}${additionalInfo}`];
    ...
    getInfoString() {
        if (!settings.store.includeNexcordInfoWhenCopying) return "";

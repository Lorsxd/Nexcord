...
import { CONTRIB_ROLE_ID, Devs, DONOR_ROLE_ID, KNOWN_ISSUES_CHANNEL_ID, REGULAR_ROLE_ID, SUPPORT_CATEGORY_ID, SUPPORT_CHANNEL_ID, VENBOT_USER_ID, NEXCORD_GUILD_ID } from "@utils/constants";
...
import { Channel, RenderModalProps } from "@nexcord/discord-types";
...
const AdditionalAllowedChannelIds = [
    "1024286218801926184", // Nexcord > #bot-spam
];
...
async function generateDebugInfoMessage() {
    ...
    const info = {
        Nexcord:
            `v${VERSION} • [${gitHash}](<https://github.com/Vendicated/Nexcord/commit/${gitHash}>)` +
    ...
    const commonIssues = {
        ...
        "Nexcord DevBuild": !IS_STANDALONE,
...
function DevBuildConfirmModal(props: RenderModalProps) {
    ...
    return (
        ...
        >
            <div>
                <Forms.FormText>You are using a custom build of Nexcord, which we do not provide support for!</Forms.FormText>

                <Forms.FormText className={Margins.top8}>
                    We only provide support for <Link href="https://nexcord.dev/download">official builds</Link>.
                    Either <Link href="https://nexcord.dev/download">switch to an official build</Link> or figure your issue out yourself.
                </Forms.FormText>
...
export default definePlugin({
    ...
    flux: {
        async CHANNEL_SELECT({ channelId }) {
            ...
            if (!IS_UPDATER_DISABLED) {
                ...
                if (isOutdated) {
                    openModal(props => (
                        ...
                        >
                            <div>
                                <Forms.FormText>You are using an outdated version of Nexcord! Chances are, your issue is already fixed.</Forms.FormText>
                                <Forms.FormText className={Margins.top8}>
            ...
            const roles = GuildMemberStore.getSelfMember(NEXCORD_GUILD_ID)?.roles;
            ...
            if (!IS_WEB && IS_UPDATER_DISABLED) {
                openModal(props => (
                    ...
                    >
                        <div>
                            <Forms.FormText>You are using an externally updated Nexcord version, which we do not provide support for!</Forms.FormText>
                            <Forms.FormText className={Margins.top8}>
                                Please either switch to an <Link href="https://nexcord.dev/download">officially supported version of Nexcord</Link>, or
    ...
    renderMessageAccessory(props) {
        ...
        if (props.channel.parent_id === SUPPORT_CATEGORY_ID && PermissionStore.can(PermissionsBits.SEND_MESSAGES, props.channel)) {
            if (props.message.content.includes("/nexcord-debug") || props.message.content.includes("/nexcord-plugins")) {
                buttons.push(
                    <Button
                        key="vc-dbg"
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        return (
            <Card variant="warning" className={Margins.top8} defaultPadding>
                Please do not private message Nexcord plugin developers for support!
                <br />
                Instead, use the Nexcord support channel: {Parser.parse("https://discord.com/channels/1015060230222131221/1026515880080842772")}

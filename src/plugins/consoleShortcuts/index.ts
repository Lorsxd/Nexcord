...
function makeShortcuts() {
    ...
    return {
        ...
        wpPatcher: { getter: () => Nexcord.WebpackPatcher },
        wpInstances: { getter: () => Nexcord.WebpackPatcher.allWebpackInstances },
        wpsearch: search,
        wpex: extract,
        ...
        PluginsApi: { getter: () => Nexcord.Plugins },
        plugins: { getter: () => Nexcord.Plugins.plugins },
        Settings: { getter: () => Nexcord.Settings },
        Api: { getter: () => Nexcord.Api },
        Util: { getter: () => Nexcord.Util },
            ...
            if (!win.prepared) {
                ...
                [...document.querySelectorAll("style"), ...document.querySelectorAll("link[rel=stylesheet]")].forEach(s => {
                    ...
                    if (s.parentElement?.tagName === "HEAD")
                        doc.head.append(n);
                    else if (n.id?.startsWith("nexcord-") || n.id?.startsWith("vcd-"))
        ...
        preEnable: (plugin: string) => (Nexcord.Settings.plugins[plugin] ??= { enabled: true }).enabled = true,
...
function loadAndCacheShortcut(key: string, val: any, forceLoad: boolean) {
    ...
    function unwrapProxy(value: any) {
        ...
        } else if (value.$$nexcordGetWrappedComponent) {
            return forceLoad ? value.$$nexcordGetWrappedComponent() : value;
        }
...
export default definePlugin({
    ...
    start() {
        ...
        if (!IS_WEB) {
            const Native = NexcordNative.pluginHelpers.ConsoleShortcuts as PluginNative<typeof import("./native")>;

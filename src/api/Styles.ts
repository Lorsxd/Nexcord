...
export const styleMap = window.NexcordStyles ??= new Map();

export const nexcordRootNode = document.createElement("nexcord-root");
/**
 * Houses all Nexcord core styles. This includes all imported css files
 */
export const coreStyleRootNode = document.createElement("nexcord-styles");
/**
 * Houses all plugin specific managed styles
 */
export const managedStyleRootNode = document.createElement("nexcord-managed-styles");
/**
 * Houses the user's themes and quick css
 */
export const userStyleRootNode = document.createElement("nexcord-user-styles");

nexcordRootNode.style.display = "none";
nexcordRootNode.append(coreStyleRootNode, managedStyleRootNode, userStyleRootNode);

export function initStyles() {
    const osValuesNode = createAndAppendStyle("nexcord-os-theme-values", coreStyleRootNode);
    createAndAppendStyle("nexcord-text", coreStyleRootNode).textContent = generateTextCss();
    const rendererCssNode = createAndAppendStyle("nexcord-css-core", coreStyleRootNode);
    const vesktopCssNode = IS_VESKTOP ? createAndAppendStyle("vesktop-css-core", coreStyleRootNode) : null;
    createAndAppendStyle("nexcord-margins", coreStyleRootNode).textContent = generateMarginCss();

    NexcordNative.native.getRendererCss().then(css => rendererCssNode.textContent = css);
    if (IS_DEV) {
        NexcordNative.native.onRendererCssUpdate(newCss => {
    ...
    NexcordNative.themes.getSystemValues().then(values => {
...
document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.append(nexcordRootNode);
}, { once: true });
...
export function enableStyle(name: string) {
    ...
    if (!style.dom) {
        ...
        style.dom.dataset.nexcordName = style.name;
    }
    compileStyle(style);

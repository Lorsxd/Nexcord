...
import { CommandArgument, CommandContext, CommandOption } from "@nexcord/discord-types";

import { sendBotMessage } from "./commandHelpers";
import { ApplicationCommandInputType, ApplicationCommandOptionType, ApplicationCommandType, NexcordCommand } from "./types";

export * from "./commandHelpers";
export * from "./types";

export let BUILT_IN: NexcordCommand[];
export const commands = {} as Record<string, NexcordCommand>;
...
export const _init = function (cmds: NexcordCommand[]) {
    try {
        BUILT_IN = cmds;
...
export const _handleCommand = function (cmd: NexcordCommand, args: CommandArgument[], ctx: CommandContext) {
    if (!cmd.isNexcordCommand)
 ...
 * @param opt
 */
export function prepareOption<O extends CommandOption | NexcordCommand>(opt: O): O {
...
// investigate
function registerSubCommands(cmd: NexcordCommand, plugin: string) {
...
export function registerCommand<C extends NexcordCommand>(command: C, plugin: string) {
    if (!BUILT_IN) {
        console.warn(
            "[CommandsAPI]",

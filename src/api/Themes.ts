...
import { ThemeStore } from "@nexcord/discord-types";
import { PopoutWindowStore } from "@webpack/common";

import { userStyleRootNode, nexcordRootNode } from "./Styles";
...
async function toggle(isEnabled: boolean) {
    if (!style) {
        if (isEnabled) {
            style = createAndAppendStyle("nexcord-custom-css", userStyleRootNode);
            NexcordNative.quickCss.addChangeListener(css => {
                style.textContent = css;
                // At the time of writing this, changing textContent resets the disabled state
                style.disabled = !Settings.useQuickCss;
                updatePopoutWindows();
            });
            style.textContent = await NexcordNative.quickCss.get();
        }
    } else
...
async function initThemes() {
    themesStyle ??= createAndAppendStyle("nexcord-themes", userStyleRootNode);
    ...
    if (IS_WEB) {
        for (const theme of enabledThemes) {
            const themeData = await NexcordNative.themes.getThemeData(theme);
            if (!themeData) continue;
            const blob = new Blob([themeData], { type: "text/css" });
            links.push(URL.createObjectURL(blob));
        }
    } else {
        const localThemes = enabledThemes.map(theme => `nexcord:///themes/${theme}?v=${Date.now()}`);
...
function applyToPopout(popoutWindow: Window | undefined, key: string) {
    ...
    doc.querySelector("nexcord-root")?.remove();

    doc.documentElement.appendChild(nexcordRootNode.cloneNode(true));
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (!IS_WEB) {
        NexcordNative.quickCss.addThemeChangeListener(initThemes);
    }
}, { once: true });

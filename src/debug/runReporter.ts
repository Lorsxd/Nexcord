    ...
    try {
        ...
        addPatch({
            ...
            replacement: {
                ...
                replace: "(Nexcord.Webpack._initReporter(),$&)"
            }
        }, "Nexcord Reporter");

        // @ts-expect-error
        Nexcord.Webpack._initReporter = function () {
        ...
        for (const [searchType, args] of Webpack.lazyWebpackSearchHistory) {
            ...
            try {
                ...
                if (result == null || (result.$$nexcordGetWrappedComponent != null && result.$$nexcordGetWrappedComponent() == null)) throw new Error("Webpack Find Fail");
            } catch (e) {
                let logMessage = searchType;
                if (method === "find" || method === "proxyLazyWebpack" || method === "LazyComponentWebpack") {
                    if (args[0].$$nexcordProps != null) {
                        logMessage += `(${args[0].$$nexcordProps.map(arg => `"${arg}"`).join(", ")})`;
                    } else {
...
// Run after the Nexcord object has been created.
// We need to add extra properties to it, and it is only created after all of Nexcord code has ran

...
async function syncSettings() {
    ...
    if (
        ...
        !await dsGet("Nexcord_cloudSecret") // this has been enabled due to local settings share or some other bug
    ) {
        ...
        showNotification({
            ...
            onClick: () => SettingsRouter.openUserSettings("nexcord_cloud_panel")
        });
        return;
    }
...
async function runUpdateCheck() {
    ...
    try {
        ...
        notify({
            title: "A Nexcord update is available!",
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Nexcord.",
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (IS_DISCORD_DESKTOP && Settings.winNativeTitleBar && IS_WINDOWS) {
        createAndAppendStyle("nexcord-native-titlebar-style", coreStyleRootNode).textContent = "[class*=titleBar]{display: none!important}";
    }
}, { once: true });

...
export async function checkForUpdates() {
    changes = await Unwrap(NexcordNative.updater.getUpdates());
...
export async function update() {
    ...
    const res = await Unwrap(NexcordNative.updater.update());

    if (res) {
        isOutdated = false;
        if (!await Unwrap(NexcordNative.updater.rebuild()))
...
export const getRepo = () => Unwrap(NexcordNative.updater.getRepo());

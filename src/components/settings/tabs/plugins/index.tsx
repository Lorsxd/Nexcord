...
function ExcludedPluginsList({ search }: { search: string; }) {
    ...
    const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "desktop" | "dev", string> = {
        ...
        dev: "Developer version of Nexcord"
    };
...
function PluginSettings() {
    ...
    const [newPlugins] = useAwaiter(() => DataStore.get("Nexcord_existingPlugins").then((cachedPlugins: Record<string, number> | undefined) => {
        ...
        DataStore.set("Nexcord_existingPlugins", existingTimestamps);
    ...
    for (const p of sortedPlugins) {
        ...
        if (isRequired) {
            const tooltipText = p.required || !depMap[p.name]
                ? "This plugin is required for Nexcord to function."

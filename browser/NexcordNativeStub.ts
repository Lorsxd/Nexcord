...
// Only import pure modules that don't import other parts of Nexcord.
...
const setCssDebounced = debounce((css: string) => NexcordNative.quickCss.set(css));

const themeStore = DataStore.createStore("NexcordThemes", "NexcordThemeData");

// probably should make this less cursed at some point
window.NexcordNative = {
    themes: {
    ...
    updater: {
        getRepo: async () => ({ ok: true, value: "https://github.com/Vendicated/Nexcord" }),
    ...
    quickCss: {
        get: () => DataStore.get("NexcordQuickCss").then(s => s ?? ""),
        set: async (css: string) => {
            await DataStore.set("NexcordQuickCss", css);
        ...
        async openEditor() {
            ...
            const win = open("about:blank", "NexcordQuickCss", features);
            if (!win) {
            ...
            win.getCurrentCss = () => NexcordNative.quickCss.get();
    ...
    settings: {
        get: () => {
            try {
                return JSON.parse(localStorage.getItem("NexcordSettings") || "{}");
            } catch (e) {
                console.error("Failed to parse settings from localStorage: ", e);
                return {};
            }
        },
        set: async (s: Settings) => localStorage.setItem("NexcordSettings", JSON.stringify(s)),

...
const commonOptions = {
    ...commonOpts,
    entryPoints: ["browser/Nexcord.ts"],
    format: "iife",
    globalName: "Nexcord",
...
const buildConfigs = [
    ...
    },
    {
        ...
        footer: { js: "//# sourceURL=file:///NexcordWeb" }
    },
    {
        ...commonOptions,
    ...
    },
    {
        ...
        outfile: "dist/Nexcord.user.js",
        banner: {
            js: readFileSync("browser/userscript.meta.js", "utf-8").replace("%version%", `${VERSION}.${new Date().getTime()}`)
        },
        footer: {
            // UserScripts get wrapped in an iife, so define Nexcord prop on window that returns our local
            js: "Object.defineProperty(unsafeWindow,'Nexcord',{get:()=>Nexcord});"
        }
    }
];
...
async function buildExtension(target, files) {
    const entries = {
        "dist/Nexcord.js": await readFile("dist/extension.js"),
        "dist/Nexcord.css": await readFile("dist/extension.css"),
...
const appendCssRuntime = readFile("dist/Nexcord.user.css", "utf-8").then(content => {
    const cssRuntime = `unsafeWindow._vcUserScriptRendererCss=\`${content.replaceAll("`", "\\`")}\``;

    return appendFile("dist/Nexcord.user.js", cssRuntime);
});

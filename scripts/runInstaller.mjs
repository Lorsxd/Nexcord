...
function getFilename() {
    switch (process.platform) {
        case "win32":
            return "NexcordInstallerCli.exe";
        case "darwin":
            return "NexcordInstaller.MacOS.zip";
        case "linux":
            return "NexcordInstallerCli-linux";
        default:
...
async function ensureBinary() {
    ...
    const outputFile = process.platform === "darwin"
        ? join(FILE_DIR, "NexcordInstaller")
        : downloadName;
    ...
    const res = await fetch(BASE_URL + filename, {
        headers: {
            "User-Agent": "Nexcord (https://github.com/Vendicated/Nexcord)",
            "If-None-Match": etag
        }
    });
    ...
    if (process.platform === "darwin") {
        ...
        logAndRun(`sudo spctl --add '${outputFile}' --label "Nexcord Installer"`);
...
try {
    execFileSync(installerBin, args, {
        ...
        env: {
            ...process.env,
            NEXCORD_USER_DATA_DIR: BASE_DIR,
            NEXCORD_DEV_INSTALL: "1"
        }
    });
} catch {

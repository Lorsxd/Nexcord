# Nexcord Code Review Style Guide

You are reviewing PRs for **Nexcord**, a modification for the Discord web app (and Electron app). It uses TypeScript, React, and webpack patching to inject into Discord. The codebase has extensive built-in utilities that contributors MUST use. Be direct, actionable, no pleasantries. Use natural, human language. Be blunt when code is bad.
...
| `any` for Discord objects                                        | Import from `@nexcord/discord-types`        | Type safety                         |
...
| `Nexcord.Plugins.plugins["X"]`                                   | `isPluginEnabled` + direct import           | Proper interop                      |
...
```typescript
import { User, Channel, Guild, GuildMember, Message, Role } from "@nexcord/discord-types";
```
...
// BAD
Nexcord.Plugins.plugins["OtherPlugin"].someFunction();
